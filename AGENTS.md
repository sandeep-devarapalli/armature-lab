...
- Publish the shareable building-vision presentation at the canonical URL `https://armaturelab.org/building-vision`. Until the React PWA replaces the legacy production site, assemble the hosted artifact with `npm run build:hosted`: preserve `site/` as the byte-identical homepage and add only the Building Vision app below `/building-vision`. Do not replace or redesign the homepage merely to publish this subpage.
